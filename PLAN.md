...
- [x] Statisch (`force-static`), OG-kaart via `opengraph-image.tsx`, `robots.ts` + `sitemap.ts`. Lighthouse mobiel gemeten op een productiebuild: **performance 93, toegankelijkheid 100, best practices 100, SEO 100** (CLS 0, LCP 3,2 s) — zie DECISIONS.md over de koude-start-uitschieter
- [x] Echte SVG-vlaggen (`public/flags/`, `npm run flags`) in plaats van emoji, omdat Windows emoji-vlaggen als twee letters toont
